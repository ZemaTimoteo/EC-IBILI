...
ConcatSignif = [signifResults{1,iii}; signifResults{2,iii}];
maxValue = max(max(ConcatSignif));

% ... plot the z_scores
for o=1:2
    subplot(1,2,o)
    imagesc (signifResults{o,iii})
    colorbar
    
    title(strcat('Trial: ', num2str(iii), ', Conditon: . ',dat.nameCondit{str2num(answer{o})}))
    axis('square');
