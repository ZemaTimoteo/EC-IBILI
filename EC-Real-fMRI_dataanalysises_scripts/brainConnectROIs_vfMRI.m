function [hf] = brainConnectROIs_vfMRI(coord, regionTag, roiSize, connStrength, showLabels, printBrain, direction )
% 
% 
% Input variables
%   coord               - Coordenates
...
widhtLine = 1; % widht of the line of plot
norm_connStrength = connStrength(:,3)/max(max(connStrength));
widhtLine = norm_connStrength*widhtLine;
...
switch (direction) 
    ...
    case 'connectivity all subs all conds'
        colMap = colormap(summer(max(roiSize)));        
end
...
%% Plotting in the brain arrows of connectivity
colMap = colormap(summer(numbOfConn));
...
for i = 1:size(connStrength,1)
    mArrow3 (coord(connStrength(i,1),:),coord(connStrength(i,2),:),'color', 'red','stemWidth',widhtLine(i));
end
